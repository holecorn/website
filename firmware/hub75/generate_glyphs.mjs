...
// The same tables for src/panelRender.js. Numbers are emitted in decimal rather than
// hex here because nothing reads this by eye — the header above is the one a
...
export const GLYPH_DIGIT_W = ${DIGIT_W};
export const GLYPH_DIGIT_H = ${DIGIT_H};
export const GLYPH_SEGMENT = [
${segRows.map((rows, i) => `  [${rows.join(', ')}], // ${ORDER[i]}`).join('\n')}
];
...
console.log(`glyphs.h + src/panelGlyphs.js: 7 segments at ${DIGIT_W}x${DIGIT_H}, ` +
