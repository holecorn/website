...
// -------------------------------------------------------------- the letters --
//
// Where each letter sits inside its board, so the splash can throw the four of them
// separately. Found here rather than laid out here: the mark is rasterised through a
// browser, so what a letter's pixels are is only knowable after the fact.
//
// A rectangle each rather than a mask each, which is what makes this cost 16 bytes a
// word instead of another 2 kB. It works because the mark happens to be four letters
// inside a box, so nothing of the box lands in a letter's rectangle and no two letters'
// rectangles meet — both checked below, because render.h's splash divides the map by
// them and would otherwise carry a slice of box off with a letter.
const level = (row, x) => (row[x >> 1] >> ((x & 1) * 4)) & 0x0f;

function letterRects(word, name) {
  const seen = new Int32Array(W * H).fill(-1);
  const parts = [];
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      if (level(word[y], x) === 0 || seen[y * W + x] >= 0) continue;
      const queue = [[x, y]];
      const part = { n: 0, x0: x, x1: x, y0: y, y1: y };
      seen[y * W + x] = parts.length;
      while (queue.length > 0) {
        const [cx, cy] = queue.pop();
        part.n++;
        part.x0 = Math.min(part.x0, cx); part.x1 = Math.max(part.x1, cx);
        part.y0 = Math.min(part.y0, cy); part.y1 = Math.max(part.y1, cy);
        for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
          const nx = cx + dx, ny = cy + dy;
          if (nx < 0 || ny < 0 || nx >= W || ny >= H) continue;
          if (level(word[ny], nx) === 0 || seen[ny * W + nx] >= 0) continue;
          seen[ny * W + nx] = parts.length;
          queue.push([nx, ny]);
        }
      }
      parts.push(part);
  ...
  if (parts.length !== 5) {
    throw new Error(
      `${name} came out as ${parts.length} pieces, not a box and four letters — ` +
        'has the mark changed, or have two letters run into each other?',
    );
  }
  // The box encloses the letters, so it is the piece with the most pixels by a wide
  // margin; the letters are numbered left to right, which is the order they read in.
  const [box, ...letters] = [...parts].sort((a, b) => b.n - a.n);
  letters.sort((a, b) => a.x0 - b.x0);
  const inside = (r, x, y) => x >= r.x0 && x <= r.x1 && y >= r.y0 && y <= r.y1;
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      if (level(word[y], x) === 0 || seen[y * W + x] !== parts.indexOf(box)) continue;
      const clash = letters.findIndex((r) => inside(r, x, y));
      if (clash >= 0) {
        throw new Error(`${name}: the box passes through letter ${clash + 1}'s rectangle at ${x},${y}`);
      }
    }
  }
  letters.forEach((a, i) => {
    letters.slice(i + 1).forEach((b, j) => {
      if (a.x0 <= b.x1 && b.x0 <= a.x1 && a.y0 <= b.y1 && b.y0 <= a.y1) {
        throw new Error(`${name}: letters ${i + 1} and ${i + j + 2} have overlapping rectangles`);
      }
    });
  });
  return letters;
...
const holeLetters = letterRects(hole, 'HOLE');
const cornLetters = letterRects(corn, 'CORN');

const rectC = (rects) => rects.map((r) => `{${r.x0}, ${r.y0}, ${r.x1}, ${r.y1}}`).join(', ');
const rectJs = (rects) =>
  rects.map((r) => `{ x0: ${r.x0}, y0: ${r.y0}, x1: ${r.x1}, y1: ${r.y1} }`).join(', ');
...
static const int LOGO_LETTERS = ${holeLetters.length};

struct LogoRect {
  int x0, y0, x1, y1;
};

static const LogoRect LOGO_HOLE_LETTERS[LOGO_LETTERS] = {${rectC(holeLetters)}};
static const LogoRect LOGO_CORN_LETTERS[LOGO_LETTERS] = {${rectC(cornLetters)}};
...
export const LOGO_LETTERS = ${holeLetters.length};

export const LOGO_HOLE_LETTERS = [${rectJs(holeLetters)}];
export const LOGO_CORN_LETTERS = [${rectJs(cornLetters)}];
